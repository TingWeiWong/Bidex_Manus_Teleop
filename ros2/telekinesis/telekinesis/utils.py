...
def transform_right_leap_pointcloud_to_camera_frame(hand_points, pose_data):
    ...
    transformed_point_cloud = np.dot(
        right_hand_points, hand_ori.T) + hand_trans
...
def transform_left_leap_pointcloud_to_camera_frame(left_hand_points, left_pose_data):
    ...
    offset = offset @ hand_ori.T
    hand_trans = update_pose_3[:3, 3].reshape(1, 3) + offset
    transformed_point_cloud_left = np.dot(
        left_hand_points, hand_ori.T) + hand_trans
...
def mask_image(image, robot0_eef, corrected_pose, left=False):
    ...
    right_hand_joint_points_homogeneous = np.hstack(
        (hand, np.ones((hand.shape[0], 1))))
    right_hand_transformed_points_homogeneous = np.dot(
        right_hand_joint_points_homogeneous, np.linalg.inv(corrected_pose).T)
    right_hand_points_to_project = right_hand_transformed_points_homogeneous[
        :, :3] / right_hand_transformed_points_homogeneous[:, [3]]
    right_hand_back_projected_points = _back_project_batch(
        right_hand_points_to_project, o3d_depth_intrinsic.intrinsic_matrix)

    right_hand_back_projected_points[:,
                                     0] = right_hand_back_projected_points[:, 0] / 1280 * 84
    right_hand_back_projected_points[:,
                                     1] = right_hand_back_projected_points[:, 1] / 720 * 84

    cv2.circle(image, (right_hand_back_projected_points[0][0],
               right_hand_back_projected_points[0][1]), 20, (255, 255, 255), -1)
...
def apply_pose_matrix(joint_positions, pose_matrix):
    homogeneous_joint_positions = np.hstack(
        [joint_positions, np.ones((joint_positions.shape[0], 1))])
...
def robot_to_hand(pose_data):
    ...
    translation_component = np.array(REALROBOT_RIGHT_HAND_OFFSET["absolute_offset"]) + \
        np.dot(
            np.array(REALROBOT_RIGHT_HAND_OFFSET["goal_ori_offset"]), goal_ori)
    pose_data_translation = goal_pos - translation_component

    return_pose_data = np.eye(4)
    return_pose_data[:3, :3] = pose_ori_matrix
    return_pose_data[:3, 3] = [-pose_data_translation[1],
                               pose_data_translation[2], -pose_data_translation[0]]
...
def robot_to_hand_left(pose_data):
    ...
    translation_component = np.array(REALROBOT_LEFT_HAND_OFFSET["absolute_offset"]) + \
        np.dot(
            np.array(REALROBOT_LEFT_HAND_OFFSET["goal_ori_offset"]), goal_ori)
    pose_data_translation = goal_pos - translation_component
...
def update_R_delta_init(frame_0_eef_pos, frame_0_eef_quat):
    ...
    canonical_t265_ori = np.dot(
        canonical_t265_ori, euler2mat(-z_angle, x_angle + 0.3, y_angle))
